...
// Updated SubscriptionPackage interface with proper date handling
interface ProcessedSubscriptionPackage extends Omit<SubscriptionPackage, 'promoStartDate' | 'promoEndDate' | 'createdAt' | 'updatedAt'> {
...
const SubscriptionPage = () => {
  ...
  const [packages, setPackages] = useState<ProcessedSubscriptionPackage[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  ...
  const loadPackages = async () => {
    try {
      ...
      // Process the packages to ensure dates are properly converted
      const processedPackages: ProcessedSubscriptionPackage[] = result.packages.map(pkg => ({
        ...pkg,
        promoStartDate: parseDate(pkg.promoStartDate),
        promoEndDate: parseDate(pkg.promoEndDate),
        createdAt: parseDate(pkg.createdAt),
        updatedAt: parseDate(pkg.updatedAt)
      }));
  ...
  const handleActionClick = (pkg: ProcessedSubscriptionPackage, e: React.MouseEvent) => {
    e.stopPropagation();
    const button = actionButtonRefs.current[pkg.id];
    if (button) {
      const rect = button.getBoundingClientRect();
