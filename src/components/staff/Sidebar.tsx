...
import { LogOut, Home, FileText, ChevronDown, Menu } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ user, onShow, setShow }) => {
  ...
  const isActive = (route: string): boolean => {
    if (route === '/staff' && pathname === '/staff') return true;
    if (route !== '/staff' && pathname.startsWith(route)) return true;
    return false;
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (sidebarRef.current && !sidebarRef.current.contains(event.target as Node)) {
        if (window.innerWidth < 768 && onShow) {
          setShow(false);
        }
      }
    };
  ...
  // Staff menu items based on user role
      ...
      subItems: [
        {
          ...
          name: 'Questionnaire',
          route: '/staff/questionnaire'
        },
        {
          id: 'verification-data',
          name: 'Verification Data',
          route: '/staff/verification-data'
        }
      ]
    },
  ];
      ...
      >
        ...
        >
          <div className="menu-item-container flex flex-col" style={{ gap: '12px' }}>
            {/* Staff Menu Items */}
            {staffMenuItems.map((item: MenuItem) => (
              <div key={item.id}>
                {item.route ? (
                  ...
                  // Menu item with submenu (Questionnaire with Verification as submodule)
